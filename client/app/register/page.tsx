...
import { z } from "zod";
import { Bell, User, Mail, Lock, Eye, EyeOff, AlertCircle, CheckCircle2 } from "lucide-react";
...
const registerSchema = z
  .object({
    ...
    whatsappNumber: z.string()
      ...
      .regex(/^\+?[1-9]\d{1,14}$/, "Must include country code (e.g., +919876543210)"),
  });
...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[var(--bg-secondary)] py-12">
            ...
            <div className="space-y-1.5 relative group">
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none transition-colors">
                  <User className={`h-[18px] w-[18px] ${errors.name ? 'text-red-500' : dirtyFields.name && !errors.name ? 'text-green-500' : 'text-gray-400 group-focus-within:text-[var(--color-primary)]'} transition-colors`} />
                </div>
                <input
                  type="text"
                  placeholder="John Doe"
                  className={`w-full pl-10 pr-4 py-2.5 bg-[var(--bg-tertiary)] border ${errors.name ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : dirtyFields.name && !errors.name ? 'border-green-500 focus:border-green-500 focus:ring-green-500/20' : 'border-[var(--border-default)] focus:border-[var(--color-primary)] focus:ring-[var(--color-primary)]/20'} rounded-xl text-[14px] font-medium text-[var(--text-primary)] placeholder-gray-400 transition-all focus:outline-none focus:ring-4`}
                  {...register("name")}
                />
              ...
              {errors.name && (
                <div className="flex items-center gap-1.5 animate-in fade-in slide-in-from-top-1">
                  ...
                  <p className="text-[12px] text-red-500 font-medium">{errors.name.message}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1.5 relative group">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-2.5 bg-[var(--bg-tertiary)] border ${errors.whatsappNumber ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : dirtyFields.whatsappNumber && !errors.whatsappNumber ? 'border-green-500 focus:border-green-500 focus:ring-green-500/20' : 'border-[var(--border-default)] focus:border-[var(--color-primary)] focus:ring-[var(--color-primary)]/20'} rounded-xl text-[14px] font-medium text-[var(--text-primary)] placeholder-gray-400 transition-all focus:outline-none focus:ring-4`}
                  {...register("whatsappNumber")}
                />
                {dirtyFields.whatsappNumber && !errors.whatsappNumber && (
                  <div className="absolute inset-y-0 right-0 pr-3.5 flex items-center pointer-events-none">
                    <CheckCircle2 className="h-[18px] w-[18px] text-green-500 animate-in fade-in" />
                  </div>
                )}
              ...
              {errors.whatsappNumber && (
                <div className="flex items-center gap-1.5 animate-in fade-in slide-in-from-top-1">
                  <AlertCircle className="h-3.5 w-3.5 text-red-500" />
                  <p className="text-[12px] text-red-500 font-medium">{errors.whatsappNumber.message}</p>
                </div>
              )}
            </div>
